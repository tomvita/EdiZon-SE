...
#define M_ENTRY_TOGGLE       \
  ...
  else if (kdown & HidNpadButton_Plus && !(kheld & HidNpadButton_ZL))
  {
    ...
    // START SEARCH
    {
      ...
      else
      {
        ...
        else if (m_searchMode == SEARCH_MODE_SAME || m_searchMode == SEARCH_MODE_DIFF  || m_searchMode == SEARCH_MODE_NOTAB || m_searchMode == SEARCH_MODE_DIFFBA || m_searchMode == SEARCH_MODE_NOTA || m_searchMode == SEARCH_MODE_SAME_A || m_searchMode == SEARCH_MODE_SAME_B || m_searchMode == SEARCH_MODE_INC || m_searchMode == SEARCH_MODE_DEC || m_searchMode == SEARCH_MODE_DIFFA || m_searchMode == SEARCH_MODE_SAMEA)
        {
          if (m_memoryDump->size() == 0)
          {
            delete m_memoryDump;
  ...
  else if (kdown & HidNpadButton_Up && !(kheld & HidNpadButton_ZL))
  {
    if (m_selectedEntry > 5)
      m_selectedEntry -= 6;
  }
  else if (kdown & HidNpadButton_Down && !(kheld & HidNpadButton_ZL))
  {
    if (m_selectedEntry < 54)
      m_selectedEntry += 6;
  }
  else if (kdown & HidNpadButton_Left && !(kheld & HidNpadButton_ZL))
  {
    if (m_selectedEntry % 6 > 0)
      m_selectedEntry--;
  }
  else if (kdown & HidNpadButton_Right && !(kheld & HidNpadButton_ZL))
  {
...
void GuiCheats::drawSearchRAMMenu()
{
  ...
  case SEARCH_VALUE:
    ...
    if (!(m_searchMode == SEARCH_MODE_SAME || m_searchMode == SEARCH_MODE_DIFF  || m_searchMode == SEARCH_MODE_NOTAB || m_searchMode == SEARCH_MODE_DIFFBA || m_searchMode == SEARCH_MODE_NOTA || m_searchMode == SEARCH_MODE_SAME_A || m_searchMode == SEARCH_MODE_SAME_B || m_searchMode == SEARCH_MODE_INC || m_searchMode == SEARCH_MODE_DEC || m_searchMode == SEARCH_MODE_DIFFA || m_searchMode == SEARCH_MODE_SAMEA) || m_use_range)
    {
      Gui::drawRectangle(300, 327, Gui::g_framebuffer_width - 600, 3, currTheme.textColor);
      if (m_searchMode == SEARCH_MODE_STRING) {
          ss << m_searchString;
          ...
          if (m_menuLocation == CANDIDATES) {
              if (m_memoryDump1 != nullptr) {  //Bookmark case
                  ...
                  if (m_memoryDump->size() == 0) {
                      ...
                      removef(EDIZON_DIR "/datadump2B.dat");
          ...
          if (m_searchMenuLocation == SEARCH_VALUE) {
              if (kdown & HidNpadButton_Up) {
                  if (m_searchMode == SEARCH_MODE_SAME) {
                      ...
                      if (newdataDump->size() == newdataDumpB->size() && newdataDump->size() != 0)
                          m_searchMode = SEARCH_MODE_SAME_A;
                      delete newdataDump;
                      delete newdataDumpB;
                  } else if (m_searchMode == SEARCH_MODE_SAME_A)
                      m_searchMode = SEARCH_MODE_SAME_B;    
                  else
                  ...
                  if (m_searchMode == SEARCH_MODE_DIFF)
                      ...
                      MemoryDump *newdataDumpB = new MemoryDump(EDIZON_DIR "/datadump2B.dat", DumpType::DATA, false);
                      if (newdataDump->size() == newdataDumpB->size() && newdataDump->size() != 0)
                          m_searchMode = SEARCH_MODE_NOTAB;
                      else
                          m_searchMode = SEARCH_MODE_DIFF;
                      delete newdataDump;
                      delete newdataDumpB;
                  } else if (m_searchMode == SEARCH_MODE_NOTAB) {
                  ...
                  } else if (m_selectedEntry == 1)  // search
                  {
                      if (m_searched) {
                          ...
                          if (m_memoryDump1 != nullptr) {
                              updatebookmark(true, false, true);
                              m_memoryDump = m_memoryDumpBookmark;
                          } else if (m_searchMode == SEARCH_MODE_SAME || m_searchMode == SEARCH_MODE_DIFF  || m_searchMode == SEARCH_MODE_NOTAB || m_searchMode == SEARCH_MODE_DIFFBA || m_searchMode == SEARCH_MODE_NOTA || m_searchMode == SEARCH_MODE_SAME_A || m_searchMode == SEARCH_MODE_SAME_B || m_searchMode == SEARCH_MODE_INC || m_searchMode == SEARCH_MODE_INC_BY || m_searchMode == SEARCH_MODE_DEC || m_searchMode == SEARCH_MODE_DEC_BY || m_searchMode == SEARCH_MODE_DIFFA || m_searchMode == SEARCH_MODE_SAMEA) {
                              if (m_memoryDump->size() == 0) {
                                  delete m_memoryDump;
                                  m_use_range = false;
                                  if (Config::getConfig()->enabletargetedscan && m_targetmemInfos.size() != 0)
...
void GuiCheats::searchMemoryMTarget(Debugger *debugger, searchValue_t searchValue1,
    ...
    auto prepareMT = [&]() {
        u32 index = 0;
        MTmap = new MT_map_t[mymap.size()];
        u32 i = 0;
        auto itr = mymap.begin();
        while (itr != mymap.end()) {
            auto temp = itr->second;
            itr->second = index;
            MTmap[i].target = itr->first;
            MTmap[i].index = index;
            i++;
            index += temp;
            ++itr;
        ...
        // store it to file
        MTfile->setMToffset(index);  // record location of mymap to MTfile
        MTfile->setMTsize(mymap.size());
        MTfile->flushBuffer();
        MTfile->putData(index*8, (u8 *)&(MTmap[0]), mymap.size()*12);
        MTfile->flushBuffer();
        printf("mymap size = %d index= %d, index+size = %ld\n",mymap.size(), index, index + mymap.size()*12);
        for (int j = 0; j < 10; j++) {
          printf ("target = %lx index = %d \n", MTmap[mymap.size()-10+j].target,MTmap[mymap.size()-10+j].index);
        }
        delete [] MTmap;
        // MTdata = new u64[index];
    };
    // if (size > 5) {
    //     size = size * 8;
    ...
    for (MemoryInfo meminfo : memInfos) {
        ...
        while (offset < meminfo.size) {
            ...
            for (u32 i = 0; i < bufferSize; i += inc_i) {
                ...
                if ((realValue._u64 != 0))
                    if (((realValue._u64 >= m_mainBaseAddr) && (realValue._u64 <= (m_mainend))) || ((realValue._u64 >= m_heapBaseAddr) && (realValue._u64 <= (m_heapEnd)))) {
                        // if (mymap.size()+0x1000 < mymap.max_size())
                        mymap[realValue._u64>>16]++;
                        // else
                        //   printf("max size approaching size = %08lX max = %08lX\n",mymap.size(),mymap.max_size());
                        ptrcount++;
                        if (ptrcount % 1000000 == 0) {
                          printf("mymap size = 0x%10lX\n",mymap.size());
                            //       writeMT();
                            //       printf("index = %d\n", index);
                            ...
                            //       u32 GT5 = 0;
                                  // u32 total = 0;
                                  // for (auto target : mymap) {
                                  //     printf("address= %10lx count= %d total= %d\n", target.first, target.second, total = total + target.second);
  ...
  while (offset < (*displayDump)->size())
  {
    ...
    for (u64 i = 0; i < bufferSize; i += sizeof(u64)) // for (size_t i = 0; i < (bufferSize / sizeof(u64)); i++)
    {
      ...
      case SEARCH_MODE_DIFF:
        ...
        break;
      case SEARCH_MODE_NOTAB:
          if ((value._s64 != prevalue._s64) && (value._s64 != valueB._s64) && ((value._s64 >= searchValue1._s64 && value._s64 <= searchValue2._s64) || !m_use_range)) {
              newDump->addData((u8 *)&address, sizeof(u64));
      ...
      case SEARCH_MODE_SAME_A:
          ...
          break;
      case SEARCH_MODE_SAME_B:
          if ((value._s64 == valueB._s64) && ((value._s64 >= searchValue1._s64 && value._s64 <= searchValue2._s64) || !m_use_range)) {
              newDump->addData((u8 *)&address, sizeof(u64));
              newdataDump->addData((u8 *)&prevalue, sizeof(u64));
              newdataDumpB->addData((u8 *)&valueB, sizeof(u64));
              newhelperinfo.count++;
...
void GuiCheats::searchMemoryValuesSecondary(Debugger *debugger, searchType_t searchType, searchMode_t searchMode, searchRegion_t searchRegion, MemoryDump **displayDump, std::vector<MemoryInfo> memInfos)
{
  ...
  MemoryDump *newMemDumpB = new MemoryDump(EDIZON_DIR "/datadump2B.dat", DumpType::DATA, true);
  ...
  for (MemoryInfo meminfo : memInfos)
  {
    ...
    while (offset < meminfo.size)
    {
      ...
      for (u64 i = 0; i < bufferSize; i += dataTypeSizes[searchType])
      {
        ...
        // }
        memcpy(&newValue, buffer + i, dataTypeSizes[searchType]);
        memcpy(&oldValue, filebuffer + i, dataTypeSizes[searchType]);
        switch (searchMode)
        {
        case SEARCH_MODE_SAME:
        ...
        case SEARCH_MODE_NOTA:
            if ((newValue._u64 != oldValue._u64) && (newValue._u64 <= m_heapBaseAddr || newValue._u64 >= (m_heapEnd))) {
                addrDump->addData((u8 *)&addr, sizeof(u64));
                newMemDump->addData((u8 *)&oldValue, sizeof(u64));
                newMemDumpB->addData((u8 *)&newValue, sizeof(u64));
                helperinfo.count++;
            }
            break;
        case SEARCH_MODE_SAMEA:
  ...
  while (offset < (*displayDump)->size())
  {
    ...
    for (u64 i = 0; i < bufferSize; i += sizeof(u64)) // for (size_t i = 0; i < (bufferSize / sizeof(u64)); i++)
    {
      ...
      case SEARCH_MODE_NOTA:
          ...
          {
              newDump->addData((u8 *)&address, sizeof(u64));
              newvalueDump->addData((u8 *)&oldvalue, sizeof(u64));
              newhelperinfo.count++;
          }
          break;
      case SEARCH_MODE_INC:
